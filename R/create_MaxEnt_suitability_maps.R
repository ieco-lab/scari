...
#'one of the smallest thresh values. If this is not the case, the extra threshold
#'will not be visible. See confusion matrix output from `compute_MaxEnt_summary_statistics()`
...
#'
#'@export
create_MaxEnt_suitability_maps <- function(model.obj, model.name, focal.species = "L_delicatula", mypath, create.dir = FALSE, env.covar.obj, describe.proj = NA, predict.type = "cloglog", clamp.pred = TRUE, thresh = NA, map.thresh = FALSE, map.thresh.extra = NA, map.style = NA, summary.file = NA) {
  ...
  for (b in predict_type) {
    ...
    model_suit_plot <- ggplot() +
      ...
      labs(title = paste0("Suitability for ", focal.species, ": ", b),
        ...
        if(!is.na(map.thresh.extra) & is.element(map.thresh.extra, thresh)) {
          ...
          if(k == map.thresh.extra) next
            ...
            model_threshold_plot <- ggplot() +
              ...
              # aesthetics
              labs(title = paste0("suitability for ", focal.species, ": cloglog | ", thresh_name, " threshold"),
                   subtitle = paste0("Model: '", model.name, "'", ifelse(clamp.pred == TRUE, ", clamped", ""), ifelse(is.na(describe.proj), "", paste0(", projected to ", describe.proj))),
                   fill = ""
              )
