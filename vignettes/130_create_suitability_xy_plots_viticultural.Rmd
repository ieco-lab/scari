...
```{r load in summary files, eval = TRUE, message = FALSE}
# summary file to extract thresholds from

# global
summary_global <- read_csv(file = file.path(mypath, "slf_global_v5", "global_summary_all_iterations.csv"))

summary_regional_ensemble <- read_csv(file = file.path(mypath, "slf_regional_ensemble_v3", "ensemble_threshold_values.csv"))
...
```{r load in suitability rasters, eval = TRUE, message = FALSE}

# global
global_1995 <- terra::rast(x = file.path(mypath, "slf_global_v5", "global_pred_suit_clamped_cloglog_globe_1981-2010_mean.asc"))

global_2055 <- terra::rast(x = file.path(mypath, "slf_global_v5", "global_pred_suit_clamped_cloglog_globe_2041-2070_GFDL_ssp_averaged.asc"))


# regional_ensemble
# historical
regional_ensemble_1995 <- terra::rast(
  x = file.path(mypath, "slf_regional_ensemble_v3", "ensemble_regional_weighted_mean_globe_1981-2010.asc")
  )
# CMIP6
## ssp 126
regional_ensemble_2055_126 <- terra::rast(
  x = file.path(mypath, "slf_regional_ensemble_v3", "ensemble_regional_weighted_mean_globe_2041-2070_GFDL_ssp126.asc")
  )
# ssp 370
regional_ensemble_2055_370 <- terra::rast(
  x = file.path(mypath, "slf_regional_ensemble_v3", "ensemble_regional_weighted_mean_globe_2041-2070_GFDL_ssp370.asc")
  )
# ssp 585
regional_ensemble_2055_585 <- terra::rast(
  x = file.path(mypath, "slf_regional_ensemble_v3", "ensemble_regional_weighted_mean_globe_2041-2070_GFDL_ssp585.asc")
  )
# ssp mean
regional_ensemble_2055_ssp_mean <- terra::rast(
  x = file.path(mypath, "slf_regional_ensemble_v3", "ensemble_regional_weighted_mean_globe_2041-2070_GFDL_ssp_averaged.asc")
  )
...
```{r load in original IVR locations dataset}

IVR_locations <- readr::read_rds(file.path(here::here(), "data", "wineries_esri54017.rds"))

```

```{r import global model suitability, message = FALSE}

# historical
xy_global_1995 <- read_csv(
  file = file.path(mypath, "slf_global_v5", "global_wineries_1981-2010_xy_pred_suit_clamped_cloglog_mean_20000m_buffer.csv")
  ) 

# CMIP6
## ssp 126
xy_global_2055_126 <- read_csv(
  file = file.path(mypath, "slf_global_v5", "global_wineries_2041-2070_GFDL_ssp126_xy_pred_suit_clamped_cloglog_mean_20000m_buffer.csv")
  ) 
## ssp 370
xy_global_2055_370 <- read_csv(
  file = file.path(mypath, "slf_global_v5", "global_wineries_2041-2070_GFDL_ssp370_xy_pred_suit_clamped_cloglog_mean_20000m_buffer.csv")
  ) 
## ssp 585
xy_global_2055_585 <- read_csv(
  file = file.path(mypath, "slf_global_v5", "global_wineries_2041-2070_GFDL_ssp585_xy_pred_suit_clamped_cloglog_mean_20000m_buffer.csv")
  ) 

```

I need to add something I am calling a join column (`join_col`). These columns are simply the main coordinate data, but rounded to a higher decimal place. This is to account for a CRS in meters UTM, which involves a very specific number. When I join the datasets downstream, they will not join correctly downstream without rounding.

Additionally, I need to rename each coordinate column to reflect its column of origin. This way, I will not lose track of which column is from which data frame among a veritable mess of joins.

```{r add join columns and rename columns- global}
...
xy_global_2055_126 <- xy_global_2055_126 %>%
    dplyr::mutate(
      join_col_x = round(x, 4),
      join_col_y = round(y, 4) 
    ) %>%
   dplyr::rename(
      x_global_2055_126 = x,
      y_global_2055_126 = y,
      "max_cloglog_suit_ssp126" = "max_cloglog_suitability_20000m_buffer" # rename max column for joining later
    ) 
...
xy_global_2055_370 <- xy_global_2055_370 %>%
   ...
   dplyr::rename(
      x_global_2055_370 = x,
      y_global_2055_370 = y,
      "max_cloglog_suit_ssp370" = "max_cloglog_suitability_20000m_buffer"
    ) 
...
xy_global_2055_585 <- xy_global_2055_585 %>%
   ...
   dplyr::rename(
      x_global_2055_585 = x,
      y_global_2055_585 = y,
      "max_cloglog_suit_ssp585" = "max_cloglog_suitability_20000m_buffer"
    ) 

```

I will do the same for the original IVR locations dataset. I will also need to join the `esri54017` IVR locations dataset, which was used to retrieve suitability information for each IVR location in vignettes 050-090, with the output from those functions, because a few points could not be resolved for suitability data. So, the predicted suitability files from my models have less points (1,074) than the original dataset (1,079), because some points did not have any data.

```{r join and save transformed IVR dataset}

# edits to IVR dataset for rounding- create a join column
IVR_locations_join <- IVR_locations %>%
    dplyr::mutate(
      join_col_x = round(x, 4),
      join_col_y = round(y, 4) # rounding to the 4s place removes only 5 more records
    )
# join col already in xy_global_1995

# join with filtering join to keep least number of common records
IVR_locations_join <- dplyr::semi_join(IVR_locations_join, xy_global_1995, by = c("join_col_x", "join_col_y")) 
# this join worked, kept 1,074 records

# add ID column
IVR_locations_final <- IVR_locations_join %>%
  # remove join columns
  dplyr::select(-c(join_col_x, join_col_y)) 

# save transformation for future use
readr::write_rds(x = IVR_locations_final, file = file.path(here::here(), "data", "wineries_esri54017_tidied.rds"))

# remove
rm(IVR_locations)
rm(IVR_locations_join)
rm(IVR_locations_final)

```

```{r re-load tidied data, eval = TRUE}
  
#read it back in
IVR_locations <- readr::read_rds(file.path(here::here(), "data", "wineries_esri54017_tidied.rds"))

```

We end up with 1,074 globally important viticultural regions.

## take mean of global model ssp scenarios 

Now, I will join all of the suitability outputs for the IVRs from the models together. I will take the mean of the suitability value within each of these predicted suitability datasets. These suitability values were taken from a 20km buffer around each viticultural area point to account for the potentially large size of these viticultural areas. We will keep the max values from those buffers. I will mean these values across ssp scenarios. 

```{r mean global ssp predictions}

# first join datasets
xy_global_2055_ssp_mean <- xy_global_2055_126 %>%
  dplyr::left_join(., xy_global_2055_370, by = c("join_col_x", "join_col_y")) %>%
  dplyr::left_join(., xy_global_2055_585, by = c("join_col_x", "join_col_y")) %>%
  dplyr::relocate(max_cloglog_suit_ssp126, max_cloglog_suit_ssp370, max_cloglog_suit_ssp585) %>%
  # take mean of columns
  dplyr::mutate(max_suit_ssp_averaged = rowMeans(.[, 1:3])) %>%
  dplyr::select(x_global_2055_126, y_global_2055_126, max_suit_ssp_averaged) %>%
  # rename for saving
  dplyr::rename(
    x = x_global_2055_126,
    y = y_global_2055_126
...
write_csv(
  ...
  file = file.path(mypath, "slf_global_v5", "global_wineries_2041-2070_GFDL_ssp_mean_xy_pred_suit_clamped_cloglog_mean_20000m_buffer.csv")
    )
...
# output file names
output_names <- c(
  "regional_ensemble_v3_wineries_1981-2010",
  "regional_ensemble_v3_wineries_2041-2070_GFDL_ssp126",
  "regional_ensemble_v3_wineries_2041-2070_GFDL_ssp370",
  "regional_ensemble_v3_wineries_2041-2070_GFDL_ssp585"
  )
...
for (a in seq_along(rasters)) {
  ...
  # import
  column_name_new <- paste0("max_cloglog_suit_", col_names[a], "_20000m")
  rast_hold <- rasters[[a]]
  ...
  xy_hold <- terra::zonal(
    ...
    wopt = list(progress = 1) # show progress bar
    ) %>%
  ...
  # old column name
  column_name_old <- names(xy_hold)[4] # get the column name of the max suitability value
  
  # rename suit column- how this works I have no clue
  xy_hold <- dplyr::rename(xy_hold, !!column_name_new := !!column_name_old)
  
  # write to csv
  write_csv(
    x = xy_hold,
    file = file.path(mypath, "slf_regional_ensemble_v3", paste0(output_names[a], "_xy_pred_suit.csv"))
  )
...
```{r re-load xy suit files, message = FALSE}

# historical
xy_regional_ensemble_1995 <- read_csv(file = file.path(mypath, "slf_regional_ensemble_v3", "regional_ensemble_v3_wineries_1981-2010_xy_pred_suit.csv"))
# CMIP6
## ssp 126
xy_regional_ensemble_2055_126 <- read_csv(file = file.path(mypath, "slf_regional_ensemble_v3", "regional_ensemble_v3_wineries_2041-2070_GFDL_ssp126_xy_pred_suit.csv"))
## ssp 370
xy_regional_ensemble_2055_370 <- read_csv(file = file.path(mypath, "slf_regional_ensemble_v3", "regional_ensemble_v3_wineries_2041-2070_GFDL_ssp370_xy_pred_suit.csv"))
## ssp 585
xy_regional_ensemble_2055_585 <- read_csv(file = file.path(mypath, "slf_regional_ensemble_v3", "regional_ensemble_v3_wineries_2041-2070_GFDL_ssp585_xy_pred_suit.csv"))

```

Perform the same edits as above for the global model

```{r add join columns and rename columns- regional_ensemble}
...
xy_regional_ensemble_2055_126 <- xy_regional_ensemble_2055_126 %>%
    dplyr::mutate(
      join_col_x = round(x, 4),
      join_col_y = round(y, 4) 
    ) %>%
   dplyr::rename(
      ...
      "max_cloglog_suit_ssp126" = "max_cloglog_suit_ssp126_20000m" # rename max column for joining later
    ) 


xy_regional_ensemble_2055_370 <- xy_regional_ensemble_2055_370 %>%
    dplyr::mutate(
      join_col_x = round(x, 4),
      join_col_y = round(y, 4) 
    ) %>%
   dplyr::rename(
      ...
      "max_cloglog_suit_ssp370" = "max_cloglog_suit_ssp370_20000m"
    ) 

xy_regional_ensemble_2055_585 <- xy_regional_ensemble_2055_585 %>%
    dplyr::mutate(
      join_col_x = round(x, 4),
      join_col_y = round(y, 4) 
    ) %>%
   dplyr::rename(
      ...
      "max_cloglog_suit_ssp585" = "max_cloglog_suit_ssp585_20000m"
    ) 

```

Now join and take the mean

```{r mean regional ensemble ssp predictions}

# first join datasets
xy_regional_ensemble_2055_ssp_mean <- xy_regional_ensemble_2055_126 %>%
  left_join(., xy_regional_ensemble_2055_370, by = c("ID", "join_col_x", "join_col_y")) %>%
  left_join(., xy_regional_ensemble_2055_585, by = c("ID", "join_col_x", "join_col_y")) %>%
  dplyr::relocate(max_cloglog_suit_ssp126, max_cloglog_suit_ssp370, max_cloglog_suit_ssp585) %>%
  # take mean of columns
  dplyr::mutate(max_suit_ssp_averaged = rowMeans(.[, 1:3])) %>%
  dplyr::select(ID, x_regional_ensemble_2055_126, y_regional_ensemble_2055_126, max_suit_ssp_averaged) %>%
  # rename for saving
  dplyr::rename(
...
write_csv(
  ...
  file = file.path(mypath, "slf_regional_ensemble_v3", "regional_ensemble_wineries_2041-2070_GFDL_ssp_mean_xy_pred_suit.csv")
  )

```

Now, I will tidy and save the data sets to .rds for our analysis.

```{r tidy datasets}

# global model historical datasets
xy_global_1995 <- xy_global_1995 %>%
  # add ID column
  ...
  dplyr::rename(
    ID = `IVR_locations[, 1]`,
    "xy_global_1995" = "max_cloglog_suitability_20000m_buffer",
    # join cols
    x = x_global_1995,
    y = y_global_1995,
    ) %>%
  dplyr::relocate(ID) %>%
  # keep only identifiers and max suit column
  dplyr::select(c(ID, x, y, xy_global_1995))
  

# global model mean future
xy_global_2055_ssp_mean <- xy_global_2055_ssp_mean %>%
  ...
  dplyr::rename(
    ID = `IVR_locations[, 1]`,
    "xy_global_2055" = "max_suit_ssp_averaged"
    ) %>%
  dplyr::relocate(ID) %>%
  dplyr::select(c(ID, x, y, xy_global_2055))
...
xy_regional_ensemble_1995 <- xy_regional_ensemble_1995 %>%
  ...
  dplyr::rename(
    ...
    "xy_regional_ensemble_1995" = "max_cloglog_suit_hist_20000m"
  ) %>%
  # remove join columns
  dplyr::select(-c(join_col_x, join_col_y)) 
...
rm(xy_global_1995)
rm(xy_global_2055_ssp_mean)
rm(xy_regional_ensemble_1995)
rm(xy_regional_ensemble_2055_ssp_mean)
...
IVR_locations <- read_rds(file.path(here::here(), "data", "wineries_esri54017_tidied.rds"))
...
```{r transform regional_ensemble xy data, eval = TRUE, warning = FALSE}
...
xy_joined <- dplyr::full_join(xy_global_1995, xy_regional_ensemble_1995, by = c("ID", "join_col_x", "join_col_y")) %>%
  ...
  # order
  dplyr::relocate(ID, join_col_x, join_col_y, xy_global_1995, xy_global_2055)
...
IVR_locations_joined <- left_join(IVR_locations, xy_joined_rescaled, by = c("ID", "join_col_x", "join_col_y")) %>%
  dplyr::relocate(ID, x, y) %>%
  # remove join cols
  dplyr::select(-c(join_col_x, join_col_y)) 
...
n_records <- nrow(IVR_locations_risk)
...
IVR_risk_table <- read_csv(file = file.path(here::here(), "vignette-outputs", "data-tables", "IVR_risk_table.csv"))
...
global_regional_risk_shift <- dplyr::mutate(
...
```{r display table results, eval = TRUE}
...
With this analysis, I found that presently, ~40% (427 of 1,074) of the global IVRs are not at of SLF establishment risk according to the global model (table 1 above, row 2). However, 38% (160 of 427) of these points are presently in quadrant 3, "high risk" (table 1 above, row 3). This means that the global model alone would label these 160 IVRs as not at risk for establishment, when in actuality they are at high risk for SLF establishment (above the MTSS threshold) when we spatially segment the presence data into an ensemble of regional-scale models. See the quadrant plot at the beginning of this vignette for the template of the plot for these results.

These results are similar under projected climate change scenarios. 47% (503 of 1,074) would be unsuitable if the global model alone were used to describe the risk of SLF. However, 36% (182 of 503) of these unsuitable IVRs are still suitable in regional_scale models and thus would be missed by an analysis of risk using only a global-scale model.
...
# add %
global_regional_risk_shift <- dplyr::mutate(global_regional_risk_shift, IVR_region_prop = scales::label_percent(accuracy = 0.01) (IVR_region_prop))

# make kable
global_regional_risk_shift <- kable(global_regional_risk_shift, format = "html", escape = FALSE) %>% 
...
# save as latex
global_regional_risk_shift_LX <- knitr::kable(x = global_regional_risk_shift, format = "latex", escape = FALSE) %>%
  kableExtra::kable_styling(bootstrap_options = "striped", full_width = FALSE)

kableExtra::save_kable(
  global_regional_risk_shift_LX, 
  file = file.path(here::here(), "vignette-outputs", "figures", "IVR_risk_plot_quadrant_props.tex"),
  self_contained = TRUE,
  bs_theme = "simplex"
  )
...
```{r display table 2, eval = TRUE}

wineries_tidied_suit <- readr::read_csv(file = file.path(here::here(), "vignette-outputs", "data-tables", "wineries_tidied_with_suit_risk.csv"))

head(wineries_tidied_suit)
...
```{r load in summary files 2}
# summary file to extract thresholds from

summary_regional_ensemble_unweighted <- read_csv(file = file.path(mypath, "slf_regional_ensemble_v3", "ensemble_threshold_values_unweighted.csv"))
...
```{r load in suitability rasters 2}

# regional_ensemble
regional_ensemble_1995_unweighted <- terra::rast(
  x = file.path(mypath, "slf_regional_ensemble_v3", "ensemble_regional_unweighted_mean_globe_1981-2010.asc")
  )

regional_ensemble_2055_unweighted <- terra::rast(
  x = file.path(mypath, "slf_regional_ensemble_v3", "ensemble_regional_unweighted_mean_globe_2041-2070_GFDL_ssp_averaged.asc")
  )
...
```{r retrieve xy suitability for regional_ensemble 2}
...
# join datasets for plotting
xy_joined_unweighted <- dplyr::full_join(xy_global_1995, xy_regional_ensemble_1995_unweighted, by = "ID") %>%
  # join CC datasets
  dplyr::full_join(., xy_global_2055, by = "ID") %>%
  dplyr::full_join(., xy_regional_ensemble_2055_unweighted, by = "ID") %>%
  dplyr::select(., -contains("join_col_")) %>%
  # order
  dplyr::relocate(ID, xy_global_1995, xy_global_2055)

```

```{r plot SLF suitability values 2, fig.asp = 1}
