...
```{r import historical xy suitability dataset and edit}

# historical
xy_global_1995 <- read_csv(
  file = file.path(mypath, "slf_global_v5", "global_slf_all_coords_1981-2010_xy_pred_suit_clamped_cloglog_mean.csv")
  ) 
...
# write to file
readr::write_rds(slf_populations_join, file.path(here::here(), "data", "slf_all_coords_final_2026-07-30_tidied.rds"))

# remove object
rm(slf_populations_join)
...
#read it back in
slf_populations <- read_rds(file.path(here::here(), "data", "slf_all_coords_final_2026-07-30_tidied.rds"))
...
```{r import other global suitability datasets, message = FALSE}

# CMIP6
## ssp 126
xy_global_2055_126 <- read_csv(
  file = file.path(mypath, "slf_global_v5", "global_slf_all_coords_2041-2070_GFDL_ssp126_xy_pred_suit_clamped_cloglog_mean.csv")
  ) 
## ssp 370
xy_global_2055_370 <- read_csv(
  file = file.path(mypath, "slf_global_v5", "global_slf_all_coords_2041-2070_GFDL_ssp370_xy_pred_suit_clamped_cloglog_mean.csv")
  ) 
## ssp 585
xy_global_2055_585 <- read_csv(
  file = file.path(mypath, "slf_global_v5", "global_slf_all_coords_2041-2070_GFDL_ssp585_xy_pred_suit_clamped_cloglog_mean.csv")
  ) 
...
write_csv(
  ...
  file = file.path(mypath, "slf_global_v5", "global_slf_all_coords_2041-2070_GFDL_ssp_mean_xy_pred_suit_clamped_cloglog_mean.csv")
    )
...
xy_regional_ensemble_2055_585 <- dplyr::left_join(xy_regional_ensemble_2055_585, slf_coordinates, by = "ID") %>%
  ...
  dplyr::relocate(ID, x_regional_ensemble_2055_585, y_regional_ensemble_2055_585)
...
slf_risk_table <- read_csv(file = file.path(here::here(), "vignette-outputs", "data-tables", "slf_risk_table.csv"))
...
With this analysis, I found that presently, 3.6% (71 of 1,961) of known SLF populations are not at risk of persisting (maintaining an established population under current climate conditions) according to the global model (table 1 above, row 2). However, 33.6% (24 of 71) of these points are presently in quadrant 3, "high risk" (table 1 above, row 3). This means that the global model alone would label these 24 SLF populations as not being at risk of persisting under present climate conditions, when they are potentially at based on our spatial segmentation of the presence data into an ensemble of regional-scale models. See the quadrant plot at the beginning of this vignette for the template of the plot for these results.

These numbers effectively double under climate change. 6.2% (123 of 1,961) would be unsuitable if the global model alone were used to describe the risk of these populations persisting. However, 27% (34 of 123) of these populations are still likely to persist according to out regional_scale models, and thus would be missed by an analysis of risk using only a global-scale model.
...
kableExtra::save_kable(
  ...
  file = file.path(here::here(), "vignette-outputs", "figures", "slf_risk_plot_quadrant_props.tex"),
  self_contained = TRUE,
  bs_theme = "simplex"
  )
