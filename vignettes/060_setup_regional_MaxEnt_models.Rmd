...
model_info <- data.frame(
  "model" = c(
    "Native (Rn) v5",
    "Invaded N American (Ri.NAmerica) v9",
    "Invaded Asian (Ri.Asia) v4"
    ),
  "train_region" = c(
...
Now I will download the Koeppen-Geiger climate zones, which I will use to clip the bioclim layers.
...
KG_zones_invaded_n_american_poly <- terra::as.polygons(
...
# write to shapefile
terra::writeVector(
  x = KG_zones_invaded_n_american_poly,
...
slf_points <- read_csv(file.path(here::here(), "vignette-outputs", "data-tables", "slf_all_coords_final_2026-07-30.csv")) %>%
...
slf_points_invaded_n_american <- terra::as.data.frame(slf_points_vect) %>%
...
KG_zones_invaded_n_american_poly <- sf::read_sf(dsn = file.path(mypath, "models", "working_dir", "shapefiles", "koeppen_climate_zones.shp")) %>%
...
ext.obj.vect <- sf::st_bbox(ext.obj.vect, crs = "ESRI:54017")
# first, crop K-G raster to N America
KG_zones_invaded_n_american_poly <- sf::st_crop(KG_zones_invaded_n_american_poly, y = ext.obj.vect) %>%
  # also get rid of ocean
  dplyr::filter(koeppen_climate_zones != 32)

# intersect
regional_invaded_n_american_withSLF <- sf::st_filter(x = KG_zones_invaded_n_american_poly, y = slf_points_invaded_n_american, join = sf::st_contains)

# write to file
sf::st_write(
  obj = regional_invaded_n_american_withSLF, 
  dsn = file.path(here::here(), "vignette-outputs", "shapefiles", "slf_regional_invaded_n_american_extent.shp"),
...
regional_invaded_n_american_withSLF <- sf::read_sf(dsn = file.path(here::here(), "vignette-outputs", "shapefiles", "slf_regional_invaded_n_american_extent.shp")) %>%
  dplyr::rename("koeppen_climate_zones" = "kppn_c_")

# get extent
st_bbox(regional_invaded_n_american_withSLF) 

# add zone nums to shapefile
regional_invaded_n_american_withSLF <- regional_invaded_n_american_withSLF %>%
...
regional_invaded_n_american_withSLF_plot <- ggplot() +
  # world map
  geom_sf(data = countries_sf, fill = "azure4", color = "black", lwd = 0.25) +
  # slf native range shapefile
  geom_sf(data = regional_invaded_n_american_withSLF, aes(fill = as.factor(koeppen_climate_zones)), color = "black", lwd = 0.25) +
  # scales
  labs(
    title = "K\U00F6ppen-Geiger climate zones as background selection area\nfor North America Invaded model",
  ...
  # scales
  scale_fill_brewer(palette = "Paired", labels = regional_invaded_n_american_withSLF$zone_name) +
  # theme
  theme_classic() +
...
regional_invaded_n_american_withSLF_plot

```

```{r save invaded_n_american K-G plot, include = FALSE}

ggsave(
  regional_invaded_n_american_withSLF_plot, 
  filename = file.path(here::here(), "vignette-outputs", "figures", "slf_regional_invaded_n_american_extent_KG_climate_zones.jpg"),
  height = 8, 
  width = 10,
  device = jpeg,
  dpi = 500
...
output.files <- list.files(path = file.path(mypath, "historical_climate_rasters", "chelsa2.1_30arcsec", "v1_maxent_10km"), pattern = "\\_global.asc$", full.names = FALSE) %>%
  # extract the 4 bioclim layers I will be using in my models. 
  grep("bio2|bio11|bio12|bio15", ., value = TRUE) %>%
  gsub(pattern = "global", replacement = "n_american")
...
```{r setup for masking bioclim layers- Ri.NAmerica}

# import layers
env.files <- list.files(path = file.path(mypath, "historical_climate_rasters", "chelsa2.1_30arcsec", "v1_maxent_10km"), pattern = "\\_n_american.asc$", full.names = TRUE) 

# output file names
output.files <- list.files(path = file.path(mypath, "historical_climate_rasters", "chelsa2.1_30arcsec", "v1_maxent_10km"), pattern = "\\_n_american.asc$", full.names = FALSE) %>%
  gsub(pattern = "n_american", replacement = "regional_invaded_n_american_KG")

# also load shapefile again
regional_invaded_n_american_withSLF <- terra::vect(file.path(here::here(), "vignette-outputs", "shapefiles", "slf_regional_invaded_n_american_extent.shp")) 
...
```{r loop to mask bioclim layers by n_american invaded range}
...
for(a in seq_along(env.files)){
  ...
  rast.hold <- terra::mask(x = rast.hold, mask = regional_invaded_n_american_withslf, overwrite = FALSE)
  # crop to extent as well
  rast.hold <- terra::crop(x = rast.hold, y = ext.obj.crop, overwrite = FALSE)
...
bio11_df <- terra::rast(x = file.path(mypath, "historical_climate_rasters", "chelsa2.1_30arcsec", "v1_maxent_10km", "bio11_1981-2010_regional_invaded_n_american_KG.asc")) %>%
  # convert to df
...
# intersect
regional_invaded_asian_withSLF <- sf::st_filter(x = KG_zones_invaded_asian_poly, y = slf_points_invaded_asian, join = sf::st_contains)
...
The coastal edges are a bit rough, but otherwise looks good!
...
# intersect
regional_native_withSLF <- sf::st_filter(x = KG_zones_native_poly, y = slf_points_native, join = sf::st_contains)
...
regional_native_withSLF_plot <- ggplot() +
  ...
  # scales
  labs(
    title = "K\U00F6ppen climate zones as background selection area for Native model",
    caption = "K-G zones selected if containing SLF presences; grey indicates zone with no SLF",
    x = "UTM Easting",
    y = "UTM Northing",
    fill = "K-G climate\nzone"
...
regional_invaded_n_american_bio11 <- raster::raster(x = file.path(mypath, "bio11_1981-2010_regional_invaded_n_american_KG.asc")) 

# load in slf points to intersect
slf_points <- read_rds(file = file.path(here::here(), "data", "slf_all_coords_final_2026-07-30.rds")) %>%
  dplyr::select(-species) %>%
  as.data.frame()
...
regional_invaded_n_american_background <- dismo::randomPoints(
  mask = regional_invaded_n_american_bio11, 
...
# save as csv
write_csv(
  x = regional_invaded_n_american_background, 
  file = file.path(here::here(), "vignette-outputs", "data-tables", "regional_invaded_n_american_background_points_v9.csv")
  )
# save as rds file
write_rds(
  regional_invaded_n_american_background, 
  file = file.path(here::here(), "data", "regional_invaded_n_american_background_points_v9.rds")
)
...
# rasters
# bg raster
regional_invaded_n_american_bio11_df <- terra::rast(x = file.path(mypath, "bio11_1981-2010_regional_invaded_n_american_KG.asc")) %>%
  terra::as.data.frame(., xy = TRUE)
  
# re-read background points 
regional_invaded_n_american_background <- read.csv(file.path(here::here(), "vignette-outputs", "data-tables", "regional_invaded_n_american_background_points_v9.csv")) 

```

```{r plot Ri.NAmerica background, eval = TRUE, echo = FALSE}

regional_invaded_n_american_background_plot <- ggplot() +
  map_style +
  # empty world map to force same aspect ratio
  ...
  # raster of interest
  geom_raster(data = regional_invaded_n_american_bio11_df, aes(x = x, y = y), fill = "azure1") +
  geom_point(data = regional_invaded_n_american_background, aes(x = x, y = y), color = "firebrick2", size = 0.25) +
  ggtitle("North America Invaded model background points") +
...
regional_invaded_n_american_background_plot
...
ggsave(
  regional_invaded_n_american_background_plot, 
  filename = file.path(here::here(), "vignette-outputs", "figures", "regional_invaded_n_american_model_background_points_v9.jpg"),
  height = 8, 
  width = 10,
  device = jpeg,
  dpi = 500
...
# save as csv
write_csv(x = regional_invaded_asian_points, file = file.path(here::here(), "vignette-outputs", "data-tables", "regional_invaded_asian_background_points_v4.csv"))
# save as rds file
write_rds(regional_invaded_asian_points, file = file.path(here::here(), "data", "regional_invaded_asian_background_points_v4.rds"))
...
# bg points
regional_invaded_asian_points <- read.csv(file = file.path(here::here(), "vignette-outputs", "data-tables", "regional_invaded_asian_background_points_v4.csv")) 
...
regional_invaded_asian_points_plot <- ggplot() +
  ...
  ggtitle("Asian Invaded model background points") +
...
ggsave(
  regional_invaded_asian_points_plot, 
  filename = file.path(here::here(), "vignette-outputs", "figures", "regional_invaded_asian_model_background_points_v4.jpg"),
  height = 8, 
  width = 10,
  device = jpeg,
  dpi = 500
...
write_csv(x = regional_native_points, 
          file = file.path(here::here(), "vignette-outputs", "data-tables", "regional_native_background_points_v5.csv")
          )
# save as rds file
write_rds(regional_native_points, 
          file = file.path(here::here(), "data", "regional_native_background_points_v5.rds")
)
...
regional_native_points <- read_csv(file = file.path(here::here(), "vignette-outputs", "data-tables", "regional_native_background_points_v5.csv")) 
...
regional_native_points_plot <- ggplot() +
  ...
  ggtitle("Native model background points") +
  coord_sf(
    xlim = c(7009531, 13605773), 
    ylim = c(-1221040, 5856473),
    expand = FALSE, #map to fill visible bounds
...
ggsave(
  regional_native_points_plot, 
  filename = file.path(here::here(), "vignette-outputs", "figures", "regional_native_model_background_points_v5.jpg"),
  height = 8, 
  width = 10,
  device = jpeg,
  dpi = 500
...
I will convert the SLF points dataset to a spatial vector object so that it can be filtered using a bounding box, then transform it back to a data frame.
...
write_csv(
  x = slf_points_invaded_n_american,
  file = file.path(here::here(), "vignette-outputs", "data-tables", "regional_invaded_n_american_train_slf_presences_v9.csv")
)
...
# load in raster
regional_invaded_n_american_df <- terra::rast(x = file.path(mypath, "historical_climate_rasters", "chelsa2.1_30arcsec", "v1_maxent_10km", "bio2_1981-2010_regional_invaded_n_american_KG.asc")) %>%
  terra::as.data.frame(., xy = TRUE)

# re-import point data
slf_points_invaded_n_american <- read_csv(file = file.path(here::here(), "vignette-outputs", "data-tables", "regional_invaded_n_american_train_slf_presences_v9.csv")) 

# plot
regional_invaded_n_american_slf_plot <- ggplot() +
  map_style +
  # empty world map to force same aspect ratio
  geom_sf(data = countries_sf, aes(geometry = geometry), fill = NA, color = NA) +
  geom_raster(data = global_bio11_df, aes(x = x, y = y), fill = "azure4") +
  geom_raster(data = regional_invaded_n_american_df, aes(x = x, y = y), fill = "azure1") +
  geom_point(data = slf_points_invaded_n_american, aes(x = x, y = y), color = "darkorange", size = 0.5) +
  labs(title = "North America Invaded model SLF presences") +
  coord_sf(
    xlim = c(-13383144, -5069425), 
    ylim = c(1914870, 6384927),
    expand = FALSE, #map to fill visible bounds
...
regional_invaded_n_american_slf_plot
...
ggsave(
  regional_invaded_n_american_slf_plot, 
  filename = file.path(here::here(), "vignette-outputs", "figures", "regional_invaded_n_american_train_slf_presences_v9.jpg"),
  height = 8, 
  width = 10,
  device = jpeg,
  dpi = 500
...
# re-import point data
slf_points_invaded_asian <- read_csv(file = file.path(here::here(), "vignette-outputs", "data-tables", "regional_invaded_asian_train_slf_presences_v4.csv")) 
...
regional_invaded_asian_slf_plot <- ggplot() +
  ...
  labs(title = "Asian Invaded model SLF presences") +
  coord_sf(crs = "ESRI:54017", datum = "ESRI:54017") +
  scale_x_continuous(limits = c(11861846, 14285214), expand = c(0, 0)) +
  scale_y_continuous(limits = c(3723782, 5227151), expand = c(0, 0))
...
ggsave(
  regional_invaded_asian_slf_plot, 
  filename = file.path(here::here(), "vignette-outputs", "figures", "regional_invaded_asian_train_slf_presences_v4.jpg"),
  height = 8, 
  width = 10,
  device = jpeg,
  dpi = 500
...
# re-import point data
slf_points_native <- read_csv(file = file.path(here::here(), "vignette-outputs", "data-tables", "regional_native_train_slf_presences_v5.csv"))
...
regional_native_slf_plot <- ggplot() +
  ...
  labs(title = "Native model SLF presences") +
  coord_sf(
    xlim = c(7009531, 13605773), 
    ylim = c(-1221040, 5856473),
    expand = FALSE, #map to fill visible bounds
...
ggsave(
  regional_native_slf_plot, 
  filename = file.path(here::here(), "vignette-outputs", "figures", "regional_native_train_slf_presences_v5.jpg"),
  height = 8, 
  width = 10,
  device = jpeg,
  dpi = 500
...
# select 20% of training total using round(nrow(slf_points_invaded_n_american) * 0.2) 

set.seed(8)

# randomly select subset of rows for testing
# size is 20% the length of the training dataset

# slf_points_invaded_n_american_test_rows <-  sample(nrow(slf_points_invaded_asian), size = round(nrow(slf_points_invaded_n_american) * 0.2), replace = FALSE) 
# filter
# slf_points_invaded_n_american_test <- slf_points_invaded_asian[slf_points_invaded_n_american_test_rows, ]

# initially, I used the above code to select 20% of the training dataset, but 20% of the training dataset was more than the entire test dataset and replacements would have been needed. So, I will just select the entire test dataset instead.

slf_points_invaded_n_american_test <- slf_points_invaded_asian

# write to csv
write_csv(
  x = slf_points_invaded_n_american_test,
  file = file.path(here::here(), "vignette-outputs", "data-tables", "regional_invaded_n_american_test_slf_presences_v9.csv")
)
...
# select 20% of training total 

set.seed(9)

# randomly select subset of rows for testing, without replacement
slf_points_invaded_asian_test_rows <-  sample(nrow(slf_points_invaded_n_american), size = round(nrow(slf_points_invaded_asian) * 0.2), replace = FALSE)
# filter
slf_points_invaded_asian_test <- slf_points_invaded_n_american[slf_points_invaded_asian_test_rows, ]
...
# randomly select subset of rows for testing, without replacement
slf_points_native_test_rows <-  sample(nrow(slf_points_invaded_n_american), size = round(nrow(slf_points_native) * 0.2), replace = FALSE) 
# filter
slf_points_native_test <- slf_points_invaded_n_american[slf_points_native_test_rows, ]
...
# validation dataset
slf_points_invaded_n_american_test <- read.csv(
  file = file.path(here::here(), "vignette-outputs", "data-tables", "regional_invaded_n_american_test_slf_presences_v9.csv")
  )
...
regional_invaded_n_american_test_plot <- ggplot() +
  map_style +
  # empty world map to force same aspect ratio
  geom_sf(data = countries_sf, aes(geometry = geometry), fill = NA, color = NA) +
  geom_raster(data = global_bio11_df, aes(x = x, y = y), fill = "azure4") +
  geom_raster(data = invaded_asian_df, aes(x = x, y = y), fill = "azure1") +
  geom_point(data = slf_points_invaded_n_american_test, aes(x = x, y = y), color = "darkorange", size = 1) +
  labs(title = "North America Invaded model validation (test) presences") +
  coord_sf(crs = "ESRI:54017", datum = "ESRI:54017") +
  scale_x_continuous(limits = c(11861846, 14285214), expand = c(0, 0)) +
  scale_y_continuous(limits = c(3723782, 5227151), expand = c(0, 0))
...
regional_invaded_n_american_withSLF_plot <- regional_invaded_n_american_withSLF_plot + 
  theme(
...
# background points
regional_invaded_n_american_background_plot <- regional_invaded_n_american_background_plot + 
  theme(
    plot.title = element_blank(),
    plot.subtitle = element_blank(),
    plot.caption = element_blank(),
...
# training presences
regional_invaded_n_american_slf_plot  <- regional_invaded_n_american_slf_plot +
  theme(
    plot.title = element_blank(),
    plot.subtitle = element_blank(),
    plot.caption = element_blank(),
...
regional_invaded_n_american_test_plot <- regional_invaded_n_american_test_plot +
  theme(
    plot.title = element_blank(),
    plot.subtitle = element_blank(),
    plot.caption = element_blank(),
...
# patchwork plot
regional_invaded_n_american_patchwork <- (
  (regional_invaded_n_american_slf_plot + regional_invaded_n_american_background_plot) / 
  (regional_invaded_n_american_withSLF_plot + regional_invaded_n_american_test_plot) 
  ) +
  plot_annotation(
    title = "North America Invaded model elements",
...
ggsave(
  regional_invaded_n_american_patchwork, 
  filename = file.path(here::here(), "vignette-outputs", "figures", "regional_invaded_n_american_model_v9_summary.jpg"),
  height = 10, 
  width = 14,
  device = jpeg,
  dpi = 500
...
regional_invaded_n_american <- terra::rast(
  x = file.path(mypath, "bio2_1981-2010_regional_invaded_n_american_KG.asc")
  ) %>%
  terra::as.data.frame(., xy = TRUE)
...
regional_invaded_asian_test_plot <- ggplot() +
  ...
  labs(title = "Asian Invaded model test presences") +
  coord_sf(
    xlim = c(-13383144, -5069425), 
    ylim = c(1914870, 6384927),
    expand = FALSE, #map to fill visible bounds
...
regional_invaded_asian_patchwork <- (
  ...
  plot_annotation(
    title = "Asian Invaded model elements",
    subtitle = "(A) training presences (B) background points (C) background selection area (D) test presences"
    )
...
ggsave(
  regional_invaded_asian_patchwork, 
  filename = file.path(here::here(), "vignette-outputs", "figures", "regional_invaded_asian_model_v4_summary.jpg"),
  height = 10, 
  width = 12,
  device = jpeg,
  dpi = 500
...
regional_native_test_plot <- ggplot() +
  ...
  labs(title = "Native model test presences") +
  coord_sf(
    xlim = c(-13383144, -5069425), 
    ylim = c(1914870, 6384927),
    expand = FALSE, #map to fill visible bounds
...
ggsave(
  regional_native_patchwork, 
  filename = file.path(here::here(), "vignette-outputs", "figures", "regional_native_model_v5_summary.jpg"),
  height = 8, 
  width = 10,
  device = jpeg,
  dpi = 500
