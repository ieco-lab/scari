...
I might begin by creating a report at the national level. To create this, you will need the correct [ISO 3166-1 country code](https://en.wikipedia.org/wiki/ISO_3166-1_alpha-3). This one comes out to be "USA", which should be entered for `locality.iso`.
...
USA_current <- usa_L_delicatula_risk_report[["risk_maps"]][["present_risk_map"]] +
...
USA_future <- usa_L_delicatula_risk_report[["risk_maps"]][["future_risk_map"]] +
  # you may need to crop the map due to outlying territories, as we have done here
  xlim(-12060785.03, -6271608.22) + # the boundaries in UTM
  ylim(3091555.56, 5614050.10)
...
```{r retrieve risk plot USA, message = FALSE, warning = FALSE}

USA_risk_plot <- usa_L_delicatula_risk_report[["viticultural_risk_plot"]] +
  labs(
    title = element_blank(),
...
```{r retrieve risk table USA, message = FALSE, warning = FALSE, results = "hide"}

# use this if you want to run this vignette locally
USA_viticultural_regions <- usa_L_delicatula_risk_report[["viticultural_regions_list"]]

# load from file optionally
...
You may begin to notice that a particular region has many records, like we can see is the case for Washington State, under the `Region` column of our risk table above. You could then produce a report only for that region, to get a better idea of the overall trend of risk shift due to climate change. We will produce a report for Washington State alone, to better visualize this trend.

The `Region` column represents states and provinces (admin-1 groups). To retrieve records for this region, we will need the [ISO 3166-2 state/province code](https://en.wikipedia.org/wiki/ISO_3166-2) which corresponds to this region. If we click this link and "USA" on the "Entry" column of the table, we find that Washington has the following iso code: "US-WA". We will enter the code into `locality.iso.2` and the name into `locality.name`.
...
scari::create_risk_report(
  ...
  locality.iso.2 = "US-WA", # the admin-1 state iso code
...
washington_current <- washington_L_delicatula_risk_report[["risk_maps"]][["present_risk_map"]] +
  labs(
    title = element_blank(),
    subtitle = element_blank(),
    caption = element_blank()
...
washington_future <- washington_L_delicatula_risk_report[["risk_maps"]][["future_risk_map"]] +
  labs(
    title = element_blank(),
    subtitle = element_blank(),
    caption = element_blank()
...
washington_risk_plot <- washington_L_delicatula_risk_report[["viticultural_risk_plot"]] +
  labs(
    title = element_blank(),
    subtitle = element_blank(),
    caption = element_blank()
...
washington_prop_area <- washington_L_delicatula_risk_report[["risk_maps_prop_area_table"]]
...
washington_risk_table <- washington_L_delicatula_risk_report[["viticultural_risk_table"]]
...
```{r retrieve SLF records using rgbif, eval = FALSE}
...
presence_data <- read_rds(file = file.path(here::here(), "data", "slf_all_coords_final_2026-07-30.rds")) %>%
...
Vignali, S., Barras, A., Braunisch, V., & Bern, C. B.-U. of. (2023). SDMtune: Species Distribution Model Selection (Version 1.3.3) [Computer software]. https://cloud.r-project.org/web/packages/SDMtune/index.html
