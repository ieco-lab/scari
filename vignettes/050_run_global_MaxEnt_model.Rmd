...
Upon inspecting the suitability maps, this model (11) initially predicted that 100% of global area was considered suitable above the MTP threshold (what we consider "minimally suitable"). This was highly unrealistic because we know that SLF has biological limitations, so I lowered the regularization to 1.5, which produced a more reasonable result, but still predicted that nearly all areas of the northern hemisphere (generally, nearly all of N America, Europe and Asia) are suitable. This is not ideal, so I again lowered the regularization multiplier to 1 (the default); this made no noticeable changes in the suitable area. I will keep the regularization multiplier at 1.5 and qpht features for this model, despite the overly broad suitable area in the Northern continents. Keep in mind that the issue with with the absolute minimum threshold of suitability, and that suitable areas based on MTSS (a much more rigorous threshold) did not change much between the two model iterations. Our framework will predict suitability based on MTSS (not MTP), so this will not affect our end results. 

The most prominent suitable areas above the MTSS (the colored areas) were predicted in North America, China, Japan, Korea, and Europe. These areas are notably where SLF is already extant, but Europe is novel in these predictions. There are also notable suitable areas in New Zealand and southern Argentina within the southern hemisphere. Here are the predicted suitability maps. Notably, as the ssp scenario becomes more severe (ssp126 -> ssp585), the predicted suitable area of each map slowly shifts towards higher latitudes.
