...
# slf presences
p_slf_points <- readr::read_rds(file = file.path(here::here(), "data", "slf_all_coords_final_2026-07-30.rds")) %>%
  dplyr::select(-species) 

# entire eastern USA background point set
a_global_background_points <- readr::read_rds(file = file.path(here::here(), "data", "global_background_points_v5.rds")) 
...
SDMtune::swd2csv(swd = global_SWD, file_name = c(
  file.path(here::here(), "vignette-outputs", "data-tables", "global_slf_presences_with_data_v5.csv"),
...
```{r grid search tunable args- for testing, echo = FALSE}

# an extra chunk for testing other tunable arguments
if (FALSE) {
  ...
  # list of hyperparameters
  hypers <- list(
    reg = c(0, 0.25, 0.5, 1, 1.5, 2, 3), # regularization multiplier
    #fc = c("h", "qpht", "lqpht") # feature classes
    iter = 10000,
    fc = "qpht"
    )
...
Based on the test AUC metric, the highest value was attributed to model 7, with a regularization multiplier of 1 and only hinge (h) features. Lets also check the TSS to be sure.
...
We get slightly different results based on the test TSS metric. We see that the highest value was attributed to the model with a regularization multiplier of 2.0 and qpht features (model 14). The two best fit models give us a choice between a regularization multiplier of 1.0 or 2.0 and all or "qpht" features. 

For previous iterations of the model, I chose model models with a reg multiplier of 2.0, and these models had an artificially inflated suitable area (there were no areas on the planet considered "unsuitable"). All areas were over the `fixed_1` threshold, which is the most conservative threshold with respect to invasive species (see 3.1 below for more detail). This means that the model was underfit (too general). When the regularization multiplier is too high, the result is an over generalized model, which fits with an overly broad predicted area of suitability. I will choose from the models using a reg multiplier of 1.5. 

The most fit model of those with a 1.5 reg multiplier according to the test AUC is model 10 (h features), while the highest of those according to the test TSS is a tie between models 11 and 12 (qpht or all features). The difference between the test TSS between models is greater than between the test AUC, so I will use the TSS. All features has been shown to be redundant (Elith et al, 2011), so I will choose use model 11, with a reg multiplier of 1.5 and qpht features.

I will also use these model settings for the three models performed for SLF at the regional scale.

```{r select optimal model}

global_model <- global_model_tuned_auc@models[[11]] # reg multiplier = 1.5, qpht features
...
Looking at the summary statistics, the most explanatory variable (according to the jackknife test for the training data, which was 80% withheld on a randomized basis) is the minimum winter temperature (Bio 11). By itself, it can produce nearly the same AUC as the full model; when left out, it has the most negative impact on the overall model. So, this is the most impactful variable in the training data and is most explanatory of SLF distributions. The randomly withheld 20% of the testing data gave the same results, both results align closely with the literature (Lee et al, 2011). The permutation importance plot validates these points.
...
knitr::include_graphics(
  path = file.path(mypath, "plots", "global_jackknife_all_iterations_training.jpg"),
...
# training
knitr::include_graphics(
  path = file.path(mypath, "plots", "global_jackknife_iteration1_testing.jpg"),
  rel_path = FALSE, # already done
  error = TRUE
  )
...
The variables behaved differently in the response curves. Looking at the univariate response curves (the result of creating a model with only that variable) for the variables based on temperature (Bio 2, mean monthly temperature seasonality and Bio 11, minimum winter temperature) both resemble a bell-shaped curve, with a peak around 8.5°C variability for Bio 2 and around -2°C minimum temp for Bio 11. Meanwhile, for the precipitation-based variables (Bio 12, annual precipitation amounts and Bio 15, precipitation seasonality), both curves suggest a threshold that trails off from its max near the beginning of the range. For Bio 12, max activity begins low and creates a skewed curve at ~1200mm precipitation, which trails off slowly, nearing 0 at about 5000mm precipitation (based on the rug plot). For Bio 15, activity below 25% precipitation seasonality is very high, with activity dropping and then rising again between 75-115%. There are no presences beyond ~115%.
...
# bio 2
knitr::include_graphics(
  path = file.path(mypath, "plots", "global_bio2_univar_resp_curve_cloglog.jpg"),
  rel_path = FALSE, # already done
  error = TRUE
  )
...
# bio 11
knitr::include_graphics(
  path = file.path(mypath, "plots", "global_bio11_univar_resp_curve_cloglog.jpg"),
  rel_path = FALSE, # already done
  error = TRUE
  )
...
# bio 12
knitr::include_graphics(
  path = file.path(mypath, "plots", "global_bio12_univar_resp_curve_cloglog.jpg"),
  rel_path = FALSE, # already done
  error = TRUE
  )
...
# bio 15
knitr::include_graphics(
  path = file.path(mypath, "plots", "global_bio15_univar_resp_curve_cloglog.jpg"),
  rel_path = FALSE, # already done
  error = TRUE
  )
...
The model itself performed well. It had a training and test AUC 0.986 and 0.977, respectively, and a training 
...
# ROC
knitr::include_graphics(
  path = file.path(mypath, "plots", "global_iteration1_ROC.jpg"),
  rel_path = FALSE, # already done
  error = TRUE
  )

print(read_csv(file.path(mypath, "global_model_eval_metrics.csv")))
...
I will use the `SDMtune::predict()` function to predict the suitability for the range of interest. I will threshold the map by the `fixed_1`, `MTSS` and `10_percentile` thresholds. The workflow for this application will use my function `create_MaxEnt_suitability_maps_CV()`.

I will take the above map and reclassify it so that areas that aren't suitable are more obvious. I will use the `MTSS` (maximum training sensitivity plus specificity threshold), as this threshold is one of the most rigorous available. It maximizes the TSS, a threshold dependent goodness of fit metric and has been shown to perform well and produce consistent predictions for presence-only methods (Liu et al, 2013; Liu et al, 2016). I will also use the `fixed_1` (Fixed cumulative value 1%) and `10_percentile` (10 percentile training threshold) thresholds to bookend the MTSS; these thresholds represent a continuum of training data exclusion. The `fixed_1` threshold excludes the bottom 1% of the training data and assumes more confidence in the validity of the training data, making the most conservative predictions (the largest suitable range for invasive species). The `10_percentile` threshold excludes the top 10% of suitable training samples, which would be more appropriate if I have less confidence in my training data. Finally, the `MTSS` threshold maximizes the sensitivity (the likelihood of detecting a true positive). (Damus, M. (2014, May 23). Threshold rule [Online post]. Google Groups: Maxent.) It should be noted that the MTP threshold excludes none of the training data, but we chose not to use this threshold because it causes artifacts in the prediction maps, such as a 0 value threshold which caused the entire study area to be considered suitable (Phillips et al, 2006).

Here is an explanation of all available threshold options for our function:

* `BTO` = Balance training omission predicted area and threshold value. This threshold is the value which best balances the predicted area and the training omission rate. It is a compromise between sensitivity and specificity (MaxEnt summary output)
* `EE` = Equate entropy of thresholded and original distributions. This threshold is the value which best equates the entropy between the original and thresholded value. Entropy is a measure of the "uniformness" of a distribution and is the situation that constrains the internal features the least (Phillips et al, 2006).
* `ETSS` = Equal training sensitivity and specificity. Equalizes the chances of missing areas of suitable distribution and assigning unsuitable distribution (Damus, 2014).
* `fixed_1` = Fixed cumulative value 1. Omits a fixed value of the bottom 1% of suitability from the training data points (MaxEnt Summary output).
* `fixed_5` = Fixed cumulative value 5. Omits a fixed value of the bottom 5% of suitability from the training data points (MaxEnt Summary output).
* `fixed_10` = Fixed cumulative value 10. Omits a fixed value of the bottom 10% of suitability from the training data points (MaxEnt Summary output).
* `MTP` or `LPT` = Minimum Training Presence. This threshold is the lowest predicted suitability value in the training presence point set (Radosavljevic, A., & Anderson, R. P. 2014).
* `MTSS` = Maximum training sensitivity plus specificity. This threshold maximizes the sum of sensitivity and specificity for the training data. We use this threshold for our main analyses (Liu et al, 2016).
* `ten_percentile` or `10_percentile` = Ten percentile training presence. This excludes the lowest 10% of predicted suitability values in the training presence point set (Radosavljevic, A., & Anderson, R. P. 2014).

In future vignettes, I will retrieve the specific value for the MTSS threshold for this model and use it to define a what is considered unsuitable in this raster. Then, I will reclassify the mean raster so that anything below this threshold is now "unsuitable", and anything above is considered suitable. I will use the mean cloglog threshold for all iterations. I will create a binary version of the mean raster, with unsuitable regions (below the MTSS training threshold) as 0, and those above the threshold as 1. Then, I can reclassify averaged raster of MaxEnt predictions. I will load in the averaged prediction for this model and reclassify the raster using a temporary raster layer that I will create. This raster will only have values where the model predicts the climate to be unsuitable, so that it can be plotted overtop the averaged raster to mask unsuitable areas.
...
scari::create_MaxEnt_suitability_maps_CV(
  ...
  clamp.pred = TRUE,
  thresh = c("fixed_1", "MTSS", "10_percentile"),
  map.thresh = TRUE, # whether thresholded versions of these maps should be created
  map.thresh.extra = "fixed_1", # whether an added underlayer threshold should be plotted (fixed 1 plotted under MTSS)
...
Upon inspecting the suitability maps, the most prominent suitable areas above the MTSS (the colored areas) were predicted in both areas where SLF has and has not already invaded. All presently known areas of SLF establishment were predicted as suitable, including eastern North America, China, Japan, and South Korea. Notable areas of predicted suitable where SLF is not yet established include most of Europe and the northwestern USA in the northern hemisphere, and southern Argentina, parts of the central Andes, New Zealand and southern Australia in the southern hemisphere. It should be noted that SLF is not presently known in the southern hemisphere. Notably, as the ssp scenario for future predictions becomes more severe (ssp126 -> ssp585), the predicted suitable area slowly shifts toward higher latitudes. Suitable areas typically grow in the northern hemisphere, and shrink in the southern hemisphere.

Initially, this model (11) initially predicted that 100% of global area was considered suitable above the MTP threshold (what we had previously considered "minimally suitable"). This was highly unrealistic because we know that SLF has biological limitations, so I lowered the regularization to 1.5, which produced a more reasonable result, but still predicted that nearly all areas of the northern hemisphere (generally, nearly all of N America, Europe and Asia) were suitable. For this reason, we made the switch to the "Fixed 1% cumulative threshold" for predicting minimal suitability. This threshold is not given to the same artifacts that the MTP is (ie, that the entire study area can be predicted as suitable if the lowest training point is considered 0 suitability), so it is a better fit (Phillips et al, 2006). This switch fixed the issue.
...
# historical data
knitr::include_graphics(
  path = file.path(mypath, "plots", "global_pred_suit_clamped_cloglog_MEAN_thresholded_MTSS_fixed_1_globe_1981-2010.jpg"),
  rel_path = FALSE, # already done
  error = TRUE
  )
...
knitr::include_graphics(
  path = file.path(mypath, "plots", "global_pred_suit_clamped_cloglog_MEAN_thresholded_MTSS_fixed_1_globe_2041-2070_GFDL_ssp126.jpg"),
  rel_path = FALSE, # already done
  error = TRUE
  )
...
# ssp126 for 2041-2070 data
knitr::include_graphics(
  path = file.path(mypath, "plots", "global_pred_suit_clamped_cloglog_MEAN_thresholded_MTSS_fixed_1_globe_2041-2070_GFDL_ssp370.jpg"),
  rel_path = FALSE, # already done
  error = TRUE
  )
...
```{r load suitability maps ssp585, eval = TRUE, echo = FALSE, message = FALSE, warning = FALSE}

# load in maps
# ssp126 for 2041-2070 data
knitr::include_graphics(
  path = file.path(mypath, "plots", "global_pred_suit_clamped_cloglog_MEAN_thresholded_MTSS_fixed_1_globe_2041-2070_GFDL_ssp585.jpg"),
  rel_path = FALSE, # already done
  error = TRUE
  )
...
Later in my analysis, I will use these point-wise suitability predictions to create a scatter plot as an index of suitability between the two scales of models (the global and an ensemble of the upcoming regional-scale models).
...
This run dropped three records, likely due to inconsistencies between the rasters which I harmonized. I will repeat the same process for the CMIP6 data.
...
readr::write_rds(summary_global, file = file.path(here::here(), "data", "global_summary_all_iterations_v5.rds"))
